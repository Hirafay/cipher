...
function displayLine() {
    ...
    switch(true) {
        case i === 2:
            bootScreen.innerHTML += `Cipher Kernel version ${remote.app.getVersion()} boot at ${Date().toString()}; root:xnu-1699.22.73~1/RELEASE_X86_64`;
        case i === 4:
...
async function displayTitleScreen() {
    ...
    bootScreen.innerHTML = "<h1>Cipher</h1>";
...
async function initUI() {
    ...
    window.term[0].term.writeln("\033[1m"+`Welcome to Cipher v${remote.app.getVersion()} - Electron v${process.versions.electron}`+"\033[0m");
...
window.openSettings = async () => {
    ...
    new Modal({
        ...
        buttons: [
            ...
            {label: "Restart Cipher", action: "remote.app.relaunch();remote.app.quit();"}
        ]
    }, () => {
